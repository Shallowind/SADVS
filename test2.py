...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.ax.clear()
        self.frame_dict = {
            0.0: {1: 'person Unknow', 2: 'person Unknow'},
            1.0: {1: 'person stand', 2: 'person carry/hold', 3: 'person carry/hold'},
            2.0: {1: 'person stand', 2: 'person carry/hold', 3: 'person carry/hold'},
            3.0: {1: 'person Unknow', 2: 'person carry/hold', 3: 'person Unknow'}
            # ... and so on
        }

        # Create a dictionary to store cumulative counts for each unique action
        action_cumulative_counts = {}

        # Get the unique actions
        unique_actions = set(action for action_dict in self.frame_dict.values() for action in action_dict.values())

        # Iterate through the frame_dict and accumulate counts for each unique action
        for t, action_dict in self.frame_dict.items():
            ...
            for order, action in action_dict.items():
                ...
                if action not in action_cumulative_counts:
                    action_cumulative_counts[action] = {'t': [], 'count': []}

            # Add entries for all unique actions, including those with count 0
            for unique_action in unique_actions:
                if unique_action not in counts:
                    counts[unique_action] = 0

                if unique_action not in action_cumulative_counts:
                    action_cumulative_counts[unique_action] = {'t': [], 'count': []}

                # Add the cumulative count for each timestamp
                action_cumulative_counts[unique_action]['t'].append(t)
                action_cumulative_counts[unique_action]['count'].append(counts[unique_action])

        # Plot each unique action as a separate line for the last 10 timestamps
        for action, data in action_cumulative_counts.items():
            last_10_data = {'t': data['t'][-10:], 'count': data['count'][-10:]}
        ...
        self.ax.set_title('Cumulative Action Counts Over Time', color='white')
    ...
    def drawLineChart(self):
        ...
        for t, action_dict in self.frame_dict.items():
            ...
            for order, action in action_dict.items():
                ...
                if action not in action_cumulative_counts:
                    action_cumulative_counts[action]['t'].append(t)
                    action_cumulative_counts[action]['count'].append(0)

                # Add the cumulative count for each timestamp
                action_cumulative_counts[action]['t'].append(t)
                action_cumulative_counts[action]['count'].append(counts[action])

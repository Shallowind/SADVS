...
class Video(QWidget):
    ...
    def setVideoOutput(self, output, video_slider=None, video_time=None, cut_time=None):
        ...
        if video_slider:
            ...
            self.video_slider.sliderMoved.connect(self.set_frame)
        if video_time:
    ...
    def play(self):
        ...
        # 视频文件
            ...
            self.timer_cv.start(int(1000 / self.fps))
    ...
    def setMedia(self, video_path=None):
        ...
        if self.video_path.isnumeric():
            self.player = cv2.VideoCapture(int(self.video_path))
        else:
            self.player = cv2.VideoCapture(self.video_path)
        self.fps = self.player.get(cv2.CAP_PROP_FPS)
        if self.fps == 0:
            self.fps = 30
        self.timer_cv.setInterval(int(self.fps))  # 1000毫秒 = 1秒

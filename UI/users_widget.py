...
from PyQt5.QtWidgets import QStyleOptionViewItem, QTableWidgetItem, QWidget, QHBoxLayout, QHeaderView
from qfluentwidgets import PushButton, setCustomStyleSheet
from qfluentwidgets import TableWidget, isDarkTheme, setTheme, Theme, TableItemDelegate
# from qfluentwidgets.components.widgets.frameless_window import FramelessWindow
from qframelesswindow import FramelessWindow, StandardTitleBar

# from UI.users_widget_ui import Ui_users_widget
...
class UserWidget(FramelessWindow):
    ...
    def __init__(self, controller):
        ...
        self.users = self.manager.get_users()
        ...
        self.tableView.setHorizontalHeaderLabels(['用户', '权限', '操作'])
    ...
    def fill_table(self):
        ...
        self.tableView.setRowCount(len(self.users))

        for i, user in enumerate(self.users.values()):
            self.tableView.setItem(i, 0, QTableWidgetItem(user.username))
            self.tableView.setItem(i, 1, QTableWidgetItem("Admin" if user.is_admin else "User"))
            action_button = PushButton("Action")
            action_button.clicked.connect(lambda: self.perform_action(user))
            self.tableView.setCellWidget(i, 2, action_button)
